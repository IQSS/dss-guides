---
title: "Installing Necessary Packages on Your Computer"
---

There are a few programs that we will need to install: Homebrew, node, ispell, and gnu-tar.

Homebrew is necessary as macOS's package manager. This will allow you to install various programs via the command line. Node.js, ispell and gnu-tar are dependencies of packages in our Emacs configuration --- ispell provides spell checking, and the other two are needed while packages are built and installed.
...
* Press enter and follow the instructions. When it finishes, Homebrew prints two lines for you to run so that the `brew` command is on your path. Copy them **from Homebrew's own output** rather than from here --- it fills in your username for you --- and run them one at a time. They look like this:
